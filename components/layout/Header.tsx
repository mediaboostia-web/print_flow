...
export default function Header() {
  ...
  return (
    <header className="h-16 bg-bg-card border-b border-border-subtle px-4 md:px-6 flex items-center justify-between sticky top-0 z-40 transition-all duration-300">
      ...
      {/* Styled Tech-Luxe Logout Confirmation Modal */}
      {isLogoutModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-bg-card border border-border-subtle rounded-3xl w-full max-w-sm p-6 shadow-premium space-y-5 text-center">
            ...
            <div className="space-y-1.5">
              <h3 className="text-base font-bold text-text-main">Déconnexion de Print_Flow</h3>
              <p className="text-xs text-text-secondary leading-relaxed">
                Êtes-vous sûr de vouloir vous déconnecter de votre espace d'imprimerie ?
              </p>
            </div>
            <div className="flex items-center gap-3 pt-2">
              <button
                onClick={() => setIsLogoutModalOpen(false)}
                className="flex-1 py-2.5 rounded-full border border-border-subtle text-xs font-bold text-text-secondary hover:bg-slate-50 dark:hover:bg-slate-800 transition"
              >
                Rester connecté
              </button>
              <button
                ...
                className="flex-1 py-2.5 rounded-full bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold transition shadow-sm"
              >
                Se déconnecter
              </button>
            </div>
